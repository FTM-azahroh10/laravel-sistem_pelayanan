...
<table id="table" class="table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{$value->tanggal}}</td>
            <td>{{$value->service}}</td>
            <td>{{$value->start_appointment}} - {{$value->end_appointment}}</td>
            <td class="text-center justify-content-center d-flex gap-3">
                <a href="/appointment-admin/{{$value->id}}/edit" class="btn text-dark">
                    <i class="fa-regular fa-pen-to-square"></i>
                </a>
                <form action="/appointment-admin/{{$value->id}}" method="post">
                    @method('delete')
                    @csrf
                    {{csrf_field()}}
