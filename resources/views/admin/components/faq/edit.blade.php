...
@endsection
@section('content')
<h4 style="color: #1d2951; margin-left: 65px"><span style="color: orange">Frequently Asked Questions (FAQ’s)</span></h4>
<div id="actions" style="background-color: #243364; padding: 10px 15px" class="d-flex justify-content-between">
    <div class="main-header ">
        <h3 style="color: white">Edit Frequently Asked Questions (FAQ’s)</h3>
    </div>
</div>

<div class="container bg-white p-4">
    <form action="/faq/{{$data->id}}" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="mb-3">
            <label for="kategori" class="form-label">Kategori</label>
            <input type="text" class="form-control" value="{{old('kategori',$data->kategori)}}" id="kategori"
                name="kategori" required>
        </div>
        <div class="mb-3">
            <label for="pertanyaan" class="form-label">Pertanyaan</label>
            <input type="text" class="form-control" id="pertanyaan" name="pertanyaan"
                value="{{old('pertanyaan',$data->pertanyaan)}}" required>
        </div>
        <div class="mb-3">
            <label for="jawaban" class="form-label">Jawaban</label>
            <input type="text" class="form-control" id="jawaban" name="jawaban"
                value="{{old('jawaban',$data->jawaban)}}" required>
        </div>
